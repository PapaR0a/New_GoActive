...
public class BWGeneralView : MonoBehaviour
{
    ...
    public void Init()
    {
        ...
        //
        m_loadingPrefab = ResourceObject.GetResource<GameObject>(BWConstant.CONFIG_PREFAB_LOADING);
        m_loadingBgPrefab = ResourceObject.GetResource<GameObject>(BWConstant.CONFIG_PREFAB_LOADING_BG);
        m_popupMessagePrefab = ResourceObject.GetResource<GameObject>(BWConstant.CONFIG_PREFAB_POPUP_MESSAGE);

        // register event
        BWControl.Api.ShowLoadingEvent += OnLoading; // register show hide loading
        BWControl.Api.ShowPopupEvent += ShowPopup; // register show popup message
    }

    void OnDestroy()
    {
        // unregister event
        BWControl.Api.ShowLoadingEvent -= OnLoading;
        BWControl.Api.ShowPopupEvent -= ShowPopup;
    }
    ...
    private void ShowPopup(BWPopupIconType iconType, string content, bool autoTurnOff, Action onClose)
    {
        ...
        go.GetComponent<BWPopupMessageView>().Init(iconType,content,autoTurnOff,onClose);
    }
}
